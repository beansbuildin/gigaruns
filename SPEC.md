...
- **Evidence of a per-ROM cooldown, not proven, not timed.** romId 2097,
  re-claimed 34 seconds after its own successful claim, failed with the
  IDENTICAL empty-error HTTP 500 seen on romId 7959's two attempts (which
  never once succeeded this session). Consistent with a gate that returns
  this same uninformative 500 whenever a ROM has nothing currently
  claimable — but the empty error body means this is inference, not a
  confirmed error code. Cooldown duration is unknown beyond ">34 seconds";
  romId 7959 may simply have already been claimed (by the user, before this
  session) rather than being broken.
...
1. **ROM enumeration** — no read endpoint found. `GET /user/me`,
   `GET /game/account/{address}`, and `GET /offchain/static` were all
   dumped and searched for any ROM-balance/list field (CLAUDE.md §2, "dump a
   related endpoint and look for it") — none found. `/offchain/static` does
   have a `romConstants` object (`{boostMultiplier: 0.6, gigusDustPerEnergy:
   10, stubBoostPerLvl: 0.1, stubBoostLvlMax: 60}`) and several `gameItems[]`
   entries with `GUIDE_CID: "Generated by certain ROMs"`, but nothing
   enumerating owned ROM NFTs or their ids. Only 7959 and 2097 are known,
   both from the user's own capture.
2. **Cooldown duration** — lower-bounded at >34s, upper bound unknown.
3. **Total daily claimable energy** — cannot be computed without (1) and
   (2). What's now known changes the framing from next.md's brief: a single
   claim delivered ~1 energy, not the 7 or 57 seen in the captured request
   bodies, so "removes the real bottleneck" is NOT yet supported by live
   evidence — it may be a small trickle per ROM rather than a large top-up.
   Needs more ROMs enumerated and more claims over time (post-cooldown) to
   size the real daily total.
