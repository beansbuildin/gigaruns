...
# RUN 4 — VERIFIED. 4/4. The day is fully spent.

```
gear 640 49→46 ✓   641 27→24 ✓   905 9→6 ✓   901 20→17 ✓   (no break, as forecast)
rings 139 36→33 ✓ sole mover, six unchanged        ledger 9→12 of 12 ✓
slot 15 = 0, slot 14 = 30 — unmoved ✓  (DISJOINTNESS, 4th test)
```

Run 4: deepest room **10**, **0/65** first-attempt failures. Shape **28/28 → 29/29**.

# THE SESSION'S SCORECARD

**ELEVEN pre-registered forecasts. ELEVEN held exactly.**

| # | forecast | result |
|---|---|---|
| 1 | 901 breaks on run 1 | ✓ |
| 2 | rod 923 reaches 0 on played cast 16 | ✓ |
| 3 | shape: Foxglove sole mover, −3 | ✓ ×4 runs |
| 4 | §71 Dendren margin stays ≤ 0 | ✓ (−3 at n=50) |
| 5–8 | run 2/3/4 gear, −3 on four pieces, no break | ✓ |
| 9–11 | ledger 3→6→9→12, disjointness ×4 | ✓ |

**Live totals.** Dungeon 4 juiced Tier-2 runs, 12/12 run-units, deaths at rooms
**14 / 8 / 7 / 10**, **0/288** first-attempt action failures, Hard Core
**19,056**, Dendren Root **2,778**. Fishing 26 played / 20 charged, server
refused cast 27, catch rate **13/26 = 50.0%**, Hard Core **4,880**.

# WHAT THE SESSION FOUND THAT NOBODY ASKED FOR

1. **§71's dichotomy is false, and the answer cost ZERO casts.** The gate the
   brief called "this session's experiment" was answerable from committed
   fixtures. **Fourth instance** of the pattern carry-forward item 6 names
   (sessions 70, 99, 123, 124) — the brief itself predicted a fourth would be
   "expected, not exceptional".
2. **The two wear sets are disjoint**, which makes every break forecastable and
   makes a rod-only repair visibly insufficient.
3. **A repair mints a new `docId`.**
4. **`--casts=N` is silently overridden by `--oil-batch`**, and the rod halt's
   safety depends on that override being small.
5. **A live instrument contradicted the settled record for a full session**
   (`checkEntryTiers.ts` on the dow-2 cell). Fixed — and the lesson applied
   immediately to this session's own new tool, whose first draft printed §71's
   now-refuted dichotomy as its footer.
6. **A mechanism I proposed was measured and rejected within minutes**, and is
   recorded as rejected so nobody re-proposes it.
7. **`Thorns`** — a new picked boon, held for a directive rather than modelled.
8. **`procEffectSize` needed its FILTER completed, not its claim relaxed** —
   the first evade+intuition co-fire in 638 exchanges.

# THE ONE CLAIM I GOT WRONG

After the rod halt I wrote that the session would get **"no third data point on
...
Corrected in place rather than deleted. **Do not fold a "therefore we cannot
learn X" onto a correct prediction — a halt is a hand-back, not the end of the
day.**
