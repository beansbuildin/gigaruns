...
/**
 * Rebuilds both reports from the full fixture corpus. Never throws — a
...
export function regenerateRunReports(config: BotConfig, log: (line: string) => void = console.log): void {
  try {
    ...
    log(`  ▸ run reports regenerated: ${dungeonRecords.length} dungeon attempts, ${fishingRecords.length} fishing casts.\n`);
  } catch (e) {
