...
| `focusMeter`/`focusMeterMax` | bobber move budget, both `3` throughout this capture — present, but un-spent-looking here since the original capture never moved the focus point more than a couple cells. **Spend rule [CONFIRMED 2026-08-15, session 13, live]**: costs the Manhattan distance from the CURRENT focus point, out of a 3-point budget that does **not** regenerate within a cast. This project's first live cast moved it `3/3 → 3/3 (dist 0) → 2/3 (dist 1) → 1/3 (dist 1)`, then a 4th move of distance 2 with only 1 point left was REJECTED outright (`HTTP 400`) — the cap is enforced server-side, not just a display number. `src/strategy/fishing/cardChoice.ts`'s `chooseCard`/`bestFocusForCard` take an optional `FocusBudget` to respect this; `src/sim/fishing/castSim.ts` does **not** model it yet, so its catch-rate figures assume unconstrained focus movement and are an optimistic ceiling relative to real Dendren. |
