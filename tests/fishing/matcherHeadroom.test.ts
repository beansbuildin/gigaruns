...
describe("matcher headroom — the scoreboard the matcher is scored against", () => {
  it("scores the stated predicate, and the predicate is the one zoneAudit uses", () => {
    ...
    expect(result.plays).toBe(869); // [session 96] was 777; [session 92] was 751  // [session 98] was 820  /* [session 99] was 860 */
    expect(result.casts).toBe(210); // [session 96] was 189; [session 92] was 178  // [session 98] was 199  /* [session 99] was 208 */
  });
  ...
  it("reproduces the four rates — floor, actual, and both ceilings", () => {
    ...
    expect(result.random).toBeCloseTo(0.20033121916842866, 3); // [session 96] was 0.198; [session 92] was 0.200  // [session 98] was 0.199
    expect(result.stayPut).toBeCloseTo(0.24165707710011508, 3); // [session 93] was 0.244; [session 92] was 0.245  // [session 98] was 0.243  /* [session 99] was 0.24069767441860465 */
    expect(result.actual).toBeCloseTo(0.37744533947065595, 3); // [session 96] was 0.375; was 0.374  // [session 98] was 0.373  /* [session 99] was 0.3767441860465116 */
    expect(result.oracleSameCard).toBeCloseTo(0.67088607594936710, 3); // [session 96] was 0.665; was 0.662  // [session 98] was 0.67  /* [session 99] was 0.6686046511627907 */
    expect(result.oracleBestCard).toBeCloseTo(0.713463751438435, 3); // [session 96] was 0.710  // [session 98] was 0.713  /* [session 99] was 0.7104651162790697 */

    // The derived readings the report prints, pinned so a change to the
    // arithmetic is caught rather than the inputs alone.
    expect(result.capturedFraction).toBeCloseTo(0.37673067661931110, 3); // [session 96] was 0.378; [session 92] was 0.373  // [session 98] was 0.37
    expect(result.headroomRemaining).toBeCloseTo(0.29344073647871116, 3); // [session 96] was 0.291; [session 91] was 0.288 // was 0.301  // [session 98] was 0.296  /* [session 99] was 0.29186046511627906 */
...
describe("the focus budget, reconstructed rather than read off the stale meter", () => {
  it("spent + remaining recovers the pre-play meter on every non-oil play", () => {
    ...
    expect(holds).toBe(845); // [session 96] was 753; [session 92] was 727  // [session 98] was 796  /* [session 99] was 836 */
...
describe("the aim-error distribution — is the miss structured or diffuse?", () => {
  it("is concentrated near the footprint rather than flat", () => {
    ...
    expect(total).toBe(514); // [session 96] was 459; [session 92] was 445  // [session 98] was 487  /* [session 99] was 509 */
    expect(h.get(1)).toBe(254); // [session 96] was 226; [session 92] was 220  // [session 98] was 243  /* [session 99] was 251 */
    expect(h.get(2)).toBe(196); // [session 96] was 171; [session 92] was 164  // [session 98] was 180  /* [session 99] was 194 */
    expect(h.get(3)).toBe(52); // [session 96] was 50; [session 91] was 46
  ...
  it("counts the plays with NO on-grid footprint — a guaranteed miss, not a bad prediction", () => {
    ...
    expect([...result.aimErrorHist.values()].reduce((a, b) => a + b, 0)).toBe(842); // [session 96] was 750; [session 92] was 726  // [session 98] was 793  /* [session 99] was 833 */
    expect([...result.aimErrorHist.values()].reduce((a, b) => a + b, 0)).toBe(result.plays - result.noFootprint);
    expect(result.perPlay.filter((p) => !p.actualHit).length).toBe(541); // [session 96] was 486; [session 92] was 470  // [session 98] was 514  /* [session 99] was 536 */
  });
});
