...
describe("the live damage economy, re-derived from the corpus", () => {
  ...
  it("lands roughly a third of its shots, for ~5 damage, against ~3 heal on a miss", () => {
    ...
    expect(LIVE.meanHeal).toBeCloseTo(3.3110273327049953 /* [session 128] was 3.303448275862069 — day 20705 */, 6); /* [session 125] was toBeLessThan(3.3) */  /* [session 126] was 3.300297324083251 */
...
describe("the BASE-DECK windows — a closed population, kept as a dated record", () => {
  ...
  it("is genuinely a different population from the rod-dealt arm — the hit rate says so", () => {
    ...
    expect(Math.abs(BASE_ARM.meanDamage - LIVE.meanDamage)).toBeCloseTo(
      0.5934131736526949, /* [session 128] was 0.5566037735849054 — +25 Dendren casts, day 20705; same direction and same reading as sessions 125/126: BASE_ARM is a CLOSED population and cannot move, so the whole movement is LIVE.meanDamage rising. Re-pinned, NOT widened. */  /* [session 126] was 0.5497474747474751 */
      6,
...
describe("the simulator's economy, same predicate", () => {
  ...
  it("THE FINDING: both drifts are negative, and the MAGNITUDE gap is a tracked measure of live-vs-sim CONVERGENCE", () => {
    ...
    expect(LIVE.drift).toBeCloseTo(-0.7426160337552743 /* [session 128] was -0.7198895027624309 — day 20705 */ /* [session 124] was -0.7104773713577185 */, /* [session 122] was -0.6881944444444444 — FIFTH move, direction arm fired, RE-DERIVED */ /* [session 121] was -0.6850220264317181 */ /* [session 118] was -0.6417445482866043 */ /* [s116b] was -0.6473354231974922 */ 6);  /* [session 116] was -0.6017241379310345 */ /* [session 113] was -0.5187436676798379 */ /* [session 102] first pin; pre-batch was -0.2426 */ /* [session 105] was -0.3504492939666239 */  /* [session 107] was -0.4330518697225573 */  /* [session 110] was -0.43875278396436523 */  /* [session 110b] was -0.5005181347150259 */  /* [session 123] was -0.7230263157894737 */  /* [session 126] was -0.7234869516935036 */
  });
