...
describe("the committed corpus", () => {
  ...
  it("reports reachability over all 188 casts", () => {
    ...
    expect(r.casts).toBe(251); // [session 96] was 189; [session 92] was 178  // [session 98] was 199  /* [session 99] was 208 */ /* [session 102] was 210 */ /* [session 105] was 230 */
    expect(r.totalDecisionPoints).toBe(1013); // [session 96] was 801 (+43); [session 92] 775 -> 799 across the ten-cast batch (+24).  // [session 98] was 844  /* [session 99] was 884 */ /* [session 102] was 893 */ /* [session 105] was 963 */
    ...
    expect(r.relaxingReachable).toBe(16); // UNCHANGED across +50 casts [session 96] /* [session 102] was 13 */ /* [session 105] was 15 */
    expect(r.focusReachable).toBe(97); // [session 96] 82 -> 84.  // [session 98] was 84 /* [session 102] was 88 */ /* [session 105] was 94 */
    expect(r.eitherReachable).toBe(105); // [session 96] 87 -> 89.  // [session 98] was 89 /* [session 102] was 93 */ /* [session 105] was 101 */
    expect(r.neitherReachable).toBe(146); // [session 96] 102 -> 110 — EIGHT of the ten new casts reach neither trigger, the same shape as session 93's nine.  // [session 98] was 110  /* [session 99] was 115 */ /* [session 102] was 117 */ /* [session 105] was 129 */
    expect(r.totalRelaxingPoints).toBe(18); // UNCHANGED across +40 casts [session 96] /* [session 102] was 15 */ /* [session 105] was 17 */
    expect(r.totalFocusPoints).toBe(293); // [session 96] 249 -> 258.  // [session 98] was 258 /* [session 102] was 271 */ /* [session 105] was 286 */
  });
  ...
  it("shows the lax definition inflating Focus reachability on the real corpus", () => {
    ...
    expect(lax.focusReachable).toBe(124); // [session 96] was 102; [session 92] 99 -> 101; see below for what the GAP did.  // [session 98] was 105 /* [session 102] was 109 */ /* [session 105] was 119 */
    ...
    expect(lax.focusReachable - strict.focusReachable).toBe(27); // [session 96] was 20; [session 92] was 18 /* [session 102] was 21 */ /* [session 105] was 25 */
    ...
    expect(lax.relaxingReachable - strict.relaxingReachable).toBe(50); // [session 96] was 29; [session 92] was 24 — the double-lethal batch added four  // [session 98] was 30 /* [session 102] was 34 */ /* [session 105] was 41 */
    expect(strict.relaxingReachable).toBe(16); // UNCHANGED across +50 casts /* [session 102] was 13 */ /* [session 105] was 15 */
  });
});
...
describe("what holding zero Mid Relaxing Oil costs — EXPECTED, not observed", () => {
  ...
  it("finds the lethal trigger reachable on 15 of 230 casts", () => {
    expect(rows).toHaveLength(251); // [session 96] was 189; [session 92] was 178  // [session 98] was 199  /* [session 99] was 208 */ /* [session 102] was 210 */ /* [session 105] was 230 */
    expect(reachable).toHaveLength(16); /* [session 102] was 13 */ /* [session 105] was 15 */
    expect(rows.reduce((n, r) => n + r.relaxingPoints, 0)).toBe(18); /* [session 102] was 15 */ /* [session 105] was 17 */
  });
  ...
  it("THE FINDING: thirteen of the fifteen were caught anyway, so only two casts could have been converted", () => {
    ...
    expect(reachable.filter((r) => r.caught)).toHaveLength(14); /* [session 102] was 11 */ /* [session 105] was 13 */
  ...
  it("prices it per oil, which is where the corpus and the sim actually agree", () => {
    ...
    expect(gained / oils).toBeCloseTo(0.125, 3); // [session 80] 2/12 -> 2/13. /* [session 102] was 0.1538 */ /* [session 105] was 0.13333333333333333 */
    ...
    expect(reachable.length).toBe(16); // the numerator itself, pinned since session 90 /* [session 102] was 13 */ /* [session 105] was 15 */
    ...
    expect((100 * reachable.length) / rows.length).toBeCloseTo(6.374501992031872, 2); // [session 96] was 6.88; was 6.915  // [session 98] was 6.53  /* [session 99] was 6.25 */ /* [session 102] was 6.190476190476191 */ /* [session 105] was 6.521739130434782 */
...
describe("the 16-cast gap, answered by MEMBERSHIP", () => {
  ...
  it("a caught cast CAN be in the gap — one does, and only via an oil-ended cast", () => {
    ...
    expect(caught).toHaveLength(108); /* [session 102] was 80; +14 over the twenty-cast batch */ // [session 96] -> 73 (+3 over the ten-cast batch); [session 93] -> 70, the single-cast batch caught its fish // [session 69] 26 -> 34; [session 72] -> 36; [session 79] -> 38; [session 80] -> 42; [session 81] -> 48; [session 90] -> 60; [session 91] -> 64 across the ten-cast batch (four catches, two of them the double-lethal firings).  // [session 98] was 73  /* [session 99] was 79 */ /* [session 105] was 94; +14 over the 21-cast day */
  ...
  it("and the gap is NOT simply 'escaped with an empty meter at the end' — that set is 74", () => {
    ...
    expect(escaped).toHaveLength(143); /* [session 102] was 130; +6 over the twenty-cast batch */ // [session 96] 119 -> 126; [session 69] 88 -> 90; [session 72] -> 92; [session 79] -> 93; [session 80] -> 98; [session 81] -> 100; [session 90] -> 108; [session 91] -> 114.  // [session 98] was 126  /* [session 99] was 129 */ /* [session 105] was 136; +7 over the 21-cast day */
    ...
    expect(terminalMeterZero).toHaveLength(93); // [session 96] 80 -> 83.  // [session 98] was 83 /* [session 102] was 86 */ /* [session 105] was 90 */
    const alreadyStrict = terminalMeterZero.filter((c) => castReachability(c, { requireTurnRemaining: true }).focusReachable);
    expect(alreadyStrict).toHaveLength(76); // [session 96] 65 -> 67.  // [session 98] was 67 /* [session 102] was 70 */ /* [session 105] was 74 */
    ...
    expect(terminalMeterZero.length - alreadyStrict.length).toBe(17); // [session 96] was 15 /* [session 105] was 16 */
