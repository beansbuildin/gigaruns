...
describe("SPEC-fishing §4 state-field claims, re-scored against the corpus", () => {
  ...
  const KNOWN_CRIT_ANOMALIES = [
    ...
    // [session 135, day 20714] +3 from the 25-cast batch. Bases 8 and 6 are not
    // new; windows [1.4375, 1.5625) and [1.4167, 1.5833) contain the standing
    // [1.500, 1.5625), so the interval is unchanged. None is lethal-clamped.
  ...
  it("all TEN exceptions fit ONE multiplier, and that interval is STILL [1.5, 1.5833)", () => {
    ...
    const observed: { base: number; actual: number }[] = [
      ...
      { base: 5, actual: 8 },
      // [session 135, day 20714] three more, none a new base; interval unchanged.
      { base: 8, actual: 12 },
      { base: 6, actual: 9 },
      { base: 6, actual: 9 },
    ];
