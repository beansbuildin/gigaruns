...
describe("SPEC-fishing §4 state-field claims, re-scored against the corpus", () => {
  ...
  const KNOWN_CRIT_ANOMALIES = [
    ...
    // [session 133] 21 -> 26, from sessions 132/133's 42 casts. Same shape as
    // every entry above: an unflagged hit landing at x1.5 (-6->-9, -4->-6),
    // or capped at the fish's remaining HP (-8->-9 from 9, -6->-9 from 9).
    "13434239 t3: card 87 hit=true crit=false predicted Δ-6, actual Δ-9 (10->1/25)",
    "13434242 t8: card 22 hit=true crit=false predicted Δ-4, actual Δ-6 (15->9/19)",
    "13450665 t1: card 88 hit=true crit=false predicted Δ-8, actual Δ-9 (9->0/14)",
    "13450749 t1: card 7 hit=true crit=false predicted Δ-6, actual Δ-9 (9->0/14)",
    "13450753 t7: card 89 hit=true crit=false predicted Δ-4, actual Δ-6 (10->4/24)",
    // [session 134, day 20713] one new anomaly from the 24-cast Golkan batch.
    // Base 5 is NOT a new base (it appears twice already) and its window
    // [7.5/5, 8.5/5) = [1.500, 1.700) CONTAINS the standing [1.500, 1.5625),
    // so the interval is unchanged. Not lethal-clamped (25 -> 17, floor is 0).
    "13466352 t3: card 51 hit=true crit=false predicted Δ-5, actual Δ-8 (25->17/30)",
  ];
  ...
  it("all TEN exceptions fit ONE multiplier, and that interval is STILL [1.5, 1.5833)", () => {
    ...
    const observed: { base: number; actual: number }[] = [
      ...
      { base: 6, actual: 9 },
      // [session 133] sessions 132/133's five. The two lethal rows use the
      // UNCLAMPED `FISH_HP_DIFF` per the rule above: 13450665 card 88 base 8
      // reads 12 (state delta clamped at 9), 13450749 card 7 base 6 reads 9.
      // Base-4 windows [5.5/4, 6.5/4) = [1.375, 1.625) and the base-6/8 rows
      // all contain the standing [1.500, 1.5625) — the interval is unchanged.
      { base: 6, actual: 9 },
      { base: 4, actual: 6 },
      { base: 8, actual: 12 },
      { base: 6, actual: 9 },
      { base: 4, actual: 6 },
      // [session 134, day 20713] card 51, base 5 -> 8. Not a new base; window
      // [1.500, 1.700) contains the standing interval, which is unchanged.
      { base: 5, actual: 8 },
    ];
  ...
  it("identifies crits by critZone geometry — and that test discriminates the zone table", () => {
    ...
    // [session 118] 86 -> 88 across the day-20698 eight-cast batch. Same
